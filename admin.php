...
class Admin extends User {
    ...
    public function insertAdminDatabase($conn) {
        // Prepare the SQL statement for inserting user
        ...
        $stmt_user->bind_param("ssss", $this->getFirstName(), $this->getLastName(), $this->getEmail(), $this->getPassword());
        ...
        // Close the user insertion statement
        $stmt_user->close();

        // Add admin role to the user in user_roles table
        $insertUserRoleSql = "INSERT INTO user_roles (UserId, RoleId) VALUES (?, (SELECT RoleId FROM roles WHERE RoleName = 'Admin'))";
        $stmt_role = $conn->prepare($insertUserRoleSql);
        if (!$stmt_role) {
        ...
        if (!$stmt_role->execute()) {
            echo "Error adding admin role to the user: " . $stmt_role->error;
            exit();
        }
        $stmt_role->close();

        // Prepare the SQL statement for inserting admin
        $sql_insert_admin = "INSERT INTO admins (UserId)
                             VALUES (?)";
        $stmt_admin = $conn->prepare($sql_insert_admin);

        // Bind parameter for admin insertion
